...
library(textmineR, quietly = TRUE)
...
The paper notes that for a probabilistic topic model, $\hat{y} = \vec{n} \odot \Theta \cdot \Phi$, where $\vec{n}$ is a vector of docuemnt lengths, $\odot$ is elementwise multiplication, $\Theta$ is a matrix of topic distributions over documents, and $\Phi$ is a matrix of token distributions over topics. 

Doing this mulitiplication explicitly in R could lead you to run out of RAM for a decently-sized corpus. (This toy example won't.) Luckily, we can use the ability to pass a list `x` and `w` to `calc_rsquared` to do that multiplication more efficiently. In that case, we set `x` $= \vec{n}\odot\Theta$ and `w` $= \Phi$.

(Note that you can pass a sparse `dgCMatrix` from the `Matrix` or something coercable to one directly to `calc_rsquared`.)
...
Note the example below doesn't use [TF-IDF](https://en.wikipedia.org/tf-idf) even though most folks recommend you do. For a more thorough example of LSA (and LDA) see `textmineR`'s [vignette on topic modeling](link.to.vignette).
...
You say you have BIG DATA?!?! Below is an example of how to chop up a calculation and have r-squared built in parallel. `mvrsquared` doesn't have a parallel back end itself. You can bring your own. (I recommend using `furrr` which is built on the wonderful `future` package.) However, what `mvrsquared` does is have the option to return only the sums of squares (specifically SSE and SST). You can calculate these in parallel/distributed and combine them by adding post-hoc. (That's a little algorithm called "map-reduce" for those in the know.) Once combined, you can calculate R-squared yourself with the cannonical formula $1 - \frac{SSE}{SST}$. 
...
Below is a toy example using `furr::future_map` to run parallel computations calculating R-squared for our LDA model, above. (Yes, this is overkill for our toy problem and it will actually be slower due to overhead. But imagine the calculations for a corpus of ~1 million documents with hundreds-of-thousands or millions of unique tokens.)

```{r}
library(furrr, quietly = TRUE)

# define what type of parallelism to use
plan(multisession) 

# divide the data and model into batches
batch_size <- 10

batches <- future_map(.x = seq(1, nrow(dtm), by = batch_size),
                      .f = function(b){
                        ...
                        list(y = y_batch,
                             x = x_batch)
                      }, .options = future_options(scheduling = 2)) # 2 cores!
